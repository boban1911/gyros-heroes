...
- **Backend (`api/`, `db/`, `lib/`).** `api/*.ts` are Vercel Functions (Node runtime) — one file per route, default export is the handler. `db/client.ts` exports a Drizzle client over the `@neondatabase/serverless` HTTP driver (works on cold starts, no connection pool to manage). `db/schema.ts` is the schema source of truth; `db/migrations/` is generated by drizzle-kit and committed (custom seed migrations like `0001_seed_loyalty_config.sql` are hand-written and registered in `_journal.json`). `lib/` holds shared server utilities — `lib/jwt.ts` (HS256 sign/verify + opaque-token hashing for magic links), `lib/email.ts` (Resend client with a stdout dev-fallback when `RESEND_API_KEY` is unset), `lib/auth.ts` (cookie-based session helpers `setCustomerSession` / `getCustomerId` / `requireCustomer`). `vercel.json` pins `api/**/*.ts` to a 10 s `maxDuration`. Local dev uses `vercel dev` (port 3000) to serve functions alongside Vite (port 5173) — never hit Vite directly for `/api/*`.

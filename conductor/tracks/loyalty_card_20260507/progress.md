...
- [~] **4. Staff PWA + scan endpoint** — *partially done — backend half landed*. Done: `api/staff/{login,logout,me,scan}.ts`, `api/wallet/google/qr-token.ts`, staff session helpers (`setStaffSession` / `getStaffPrincipal` / `getStaffId` / `requireStaff` / `requireAdmin` / `clearStaffSession`) in `lib/auth.ts`, QR token JWT helpers (`signQrToken` / `verifyQrToken`) in `lib/jwt.ts`, `scripts/seed-staff-user.ts` (+ `npm run seed:staff` alias). Pending: scan PWA Vite entry (`scan/index.html`, `src/scan/`), tests, and `vite.config.ts` wiring — separate agents.
